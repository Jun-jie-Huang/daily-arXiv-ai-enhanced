...
### [58] [TokenShapley: Token Level Context Attribution with Shapley Value](https://arxiv.org/abs/2507.05261)
*Yingtai Xiao,Yuqing Zhu,Sirat Samyoun,Wanrong Zhang,Jiachen T. Wang,Jian Du*

Main category: cs.CL

TL;DR: 该论文提出了一种名为TokenShapley的细粒度令牌级归因方法，以验证大型语言模型生成回答的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能做到句子级归因，无法准确追踪回答中特定关键词（如数字、年份、名称）的来源，因此需要一种更细粒度的归因技术。

Method: 提出TokenShapley方法，将Shapley值数据归因与KNN检索技术结合，利用预计算的数据存储库进行上下文检索，并通过计算Shapley值量化每个令牌的重要性。

Result: 在四个基准测试上，TokenShapley在令牌级归因准确率上比现有最先进方法提升了11%至23%。

Conclusion: TokenShapley实现了更精细的令牌级数据归因，有效提升了大型语言模型生成内容的可验证性和准确性。
...
### [59] [User Behavior Prediction as a Generic, Robust, Scalable, and Low-Cost Evaluation Strategy for Estimating Generalization in LLMs](https://arxiv.org/abs/2507.05266)
*Sougata Saha,Monojit Choudhury*

Main category: cs.CL

TL;DR: 此论文提出用户行为预测作为衡量大型语言模型（LLMs）泛化能力的替代方法，避免数据泄露问题，并在推荐系统数据集上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统使用知识检索和推理任务来衡量LLMs的泛化能力存在数据污染问题，随着模型规模增长，确保测试任务未被训练过变得不可行。作者认为需要一个更稳健且可扩展的评价指标。

Method: 提出以用户行为预测作为泛化能力的测量框架，并在电影和音乐推荐数据集上对GPT-4o、GPT-4o-mini及Llama-3.1-8B-Instruct进行测试。

Result: 实验结果符合预测，GPT-4o表现优于GPT-4o-mini和Llama模型，但所有模型特别是Llama仍有较大提升空间。

Conclusion: 用户行为预测为评估大型语言模型的泛化能力提供了一个理论合理且可扩展的替代方案，未来仍需进一步提升模型性能。
...
### [60] [An Adaptive Supervised Contrastive Learning Framework for Implicit Sexism Detection in Digital Social Networks](https://arxiv.org/abs/2507.05271)
*Mohammad Zia Ur Rehman,Aditya Shah,Nagendra Kumar*

Main category: cs.CL

TL;DR: 提出了一种适应性监督对比学习框架ASCEND，用于检测隐性性别歧视，通过阈值控制的对比学习优化文本表示，显著提升检测效果。


<details>
  <summary>Details</summary>
Motivation: 社会媒体上的隐性性别歧视内容难以被传统方法准确检测，需要更有效的表示学习方法来捕捉细微语言特征。

Method: 引入基于阈值的对比学习机制，通过计算文本嵌入的余弦相似度，仅对相似度超过可学习阈值的样本对进行正样本处理，结合对比损失和交叉熵损失共同训练，辅以词级注意力机制和情感、情绪及有害性特征。

Result: 在EXIST2021和MLSC数据集上，ASCEND在多个任务中平均宏F1分别提升了9.86%、29.63%和32.51%，明显优于现有方法。

Conclusion: ASCEND有效捕捉隐性性别歧视语言中的细微线索，显著提升了检测准确性，证明了其在隐性性别歧视检测领域的实用价值。
...
TL;DR: 本文提出了一种结合情感分析、提示工程和跨模态注意力融合的AI框架，显著提升了远程教育中学生辍学预测的准确率，并能生成可解释的干预策略。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型仅用结构化数据，难以捕捉学生互动中的情感和上下文因素，导致辍学预测效果不佳。

Method: 利用检索增强生成（RAG）技术进行领域情感分析，通过优化提示工程识别学业压力源，结合跨模态注意力融合文本、行为和社会人口统计学数据，构建综合风险画像。

Result: 在4423名学生的纵向数据集上，模型准确率89%、F1分数0.88，分别超越传统模型7%和减少假阴性21%。

Conclusion: 该框架不仅提升辍学预测性能，还能基于情境生成解释性干预措施，促进预测分析与教学实践的结合，有效降低辍学风险。
...
### [62] [LCDS: A Logic-Controlled Discharge Summary Generation System Supporting Source Attribution and Expert Review](https://arxiv.org/abs/2507.05319)
*Cheng Yuan,Xinkai Rui,Yongqi Fan,Yawei Fan,Boyang Zhong,Jiacheng Wang,Weiyan Zhang,Tong Ruan*

Main category: cs.CL

TL;DR: 本论文提出了LCDS系统，旨在解决大型语言模型在生成出院小结时的内容失实和信息来源匹配困难问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动生成出院小结时存在幻觉问题，即生成不准确或无有效来源的信息，且电子病历数据冗长，难以准确归因。

Method: 提出LCDS系统，通过计算电子病历与出院小结的文本相似度构建源映射表，利用逻辑规则控制生成内容，支持内容源头归因，并结合专家反馈进行模型微调。

Result: LCDS系统能够生成更可靠的银标准出院小结，确保内容准确并便于专家审查和纠错，提升了出院小结生成的质量。

Conclusion: LCDS通过逻辑控制和源头归因有效缓解了大型语言模型的幻觉问题，为自动生成医学出院小结提供了实用且可靠的解决方案。
...
### [63] [MindFlow: Revolutionizing E-commerce Customer Support with Multimodal LLM Agents](https://arxiv.org/abs/2507.05330)
*Ming Gong,Xucheng Huang,Chenghan Yang,Xianhan Peng,Haoxin Wang,Yang Liu,Ling Jiang*

Main category: cs.CL

TL;DR: MindFlow是首个针对电商的开源多模态大语言模型代理，集成记忆、决策与行动模块，显著提升复杂查询处理和用户满意度。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在复杂多模态电商服务场景中能力有限，亟需提升智能交互性能。

Method: 基于CoALA框架，MindFlow采用模块化的“MLLM作为工具”策略，融合记忆、决策和行动模块，实现视觉与文本的推理。

Result: 在线A/B测试和模拟消融实验表明，MindFlow在复杂查询处理、用户满意度提升及运营成本降低方面表现优秀，真实场景中提升达93.53%。

Conclusion: MindFlow显著推动了多模态大语言模型在电商客服的应用，有效改善复杂场景下的智能服务水平。
...
### [64] [LoRA-Augmented Generation (LAG) for Knowledge-Intensive Language Tasks](https://arxiv.org/abs/2507.05346)
*William Fleshman,Benjamin Van Durme*

Main category: cs.CL

TL;DR: 本文提出了LoRA增强生成（LAG）方法，用于高效选择和组合针对特定任务和领域的微调语言模型专家，无需额外训练或数据访问，在多任务上表现优越。


<details>
  <summary>Details</summary>
Motivation: 随着大量针对特定任务和领域的微调语言模型专家的出现，亟需一种高效的选择和组合方法以充分利用这些专家资源。

Method: 提出LAG方法，通过在每个token和层级基础上，利用LoRA适配器对专家模型进行高效过滤、检索和应用，无需额外训练或数据访问。

Result: 在多种知识密集型任务中，LAG优于现有无数据方法，同时展示了与如检索增强生成（RAG）等其他解决方案的兼容性。

Conclusion: LAG是一种有效且灵活的方法，能够高效整合大量专家模型，实现优异的任务性能，且兼容其他增强生成技术。
...
### [65] [On the Bias of Next-Token Predictors Toward Systematically Inefficient Reasoning: A Shortest-Path Case Study](https://arxiv.org/abs/2507.05362)
*Riccardo Alberghi,Elizaveta Demyanenko,Luca Biggio,Luca Saglietti*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在推理任务中的计算分配和系统性推理的重要性，通过在分层图的最短路径任务中训练模型，发现训练时使用效率较低的推理路径反而能提高模型对未见图的泛化能力，这一提升与训练信号的连贯性和局部递增性相关。


<details>
  <summary>Details</summary>
Motivation: 探讨增加测试时计算量和推理的系统性、递增性如何影响大型语言模型的推理能力，特别在推理路径冗余与效率之间的权衡。

Method: 在分层图的最短路径任务中，使用定制分词器训练解码器Transformer模型，比较基于最优动态规划路径和较长有效的回溯路径的训练效果。

Result: 使用较低效但连贯的长路径训练的模型，在同样训练预算下对未见图的泛化性能更好。注入随机冗余无益甚至有害。泛化效果与模型对下一个token的预测信心相关。

Conclusion: 长且连贯的、局部递增的推理路径能够提供更易优化的训练信号，从而提升模型的泛化能力，而不是简单的路径长度或冗余。
...
### [66] [EduCoder: An Open-Source Annotation System for Education Transcript Data](https://arxiv.org/abs/2507.05385)
*Guanzhong Pan,Mei Tan,Hyunji Nam,Lucía Langlois,James Malamut,Liliana Deonizio,Dorottya Demszky*

Main category: cs.CL

TL;DR: EduCoder是一种专门支持教育对话中话语级别标注的工具，解决了教育对话转录本复杂编码的挑战，提供类别和开放式标注，支持上下文信息并促进多标注者比较校准。


<details>
  <summary>Details</summary>
Motivation: 现有通用文本标注工具难以应对教育对话中师生和同伴多样互动的复杂编码需求，需要一个专门针对教育领域对话进行标注的系统。

Method: EduCoder通过允许研究人员和领域专家基于观察数据协同定义复杂代码本，结合类别和开放式标注类型以及课时目的和教学价值等上下文信息，提供多标注者答案的并排对比功能来提高标注的可靠性。

Result: 系统有效支持了复杂教育对话的标注流程，增强了标注数据的质量和可靠性，且作为开源工具提供，附带演示视频。

Conclusion: EduCoder填补了教育对话标注工具的空白，通过协作定义复杂编码体系和多标注校准，提高了教育数据标注的有效性和可靠性。
...
### [67] [The Generalization Ridge: Information Flow in Natural Language Generation](https://arxiv.org/abs/2507.05387)
*Ruidi Chang,Chunyuan Deng,Hanjie Chen*

Main category: cs.CL

TL;DR: 本文通过信息论框架InfoRidge分析了Transformer模型中隐藏层与目标输出之间的互信息，发现中上层存在预测信息峰值，即“泛化脊”，并通过残差缩放系数验证了中间层在泛化中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现中间层比最终层生成更具泛化性的表征，但其泛化能力如何产生和在训练过程中如何传播尚不清楚。

Method: 提出InfoRidge框架，利用互信息度量隐藏表示与目标输出的相关性，追踪任务相关信息在训练中的流动；引入残差缩放系数量化各层的重要性和模型在分布转移下的行为。

Result: 实验证明预测信息在中上层达到峰值形成“泛化脊”，最终层则信息下降，残差缩放系数显示模型在分布转移下更依赖中间层，弱化最终层。

Conclusion: 中间层在Transformer模型中起到关键的泛化作用，理解其信息流动有助于深入揭示模型内部机制和改进自然语言生成任务的性能。
...
### [68] [Controlling What You Share: Assessing Language Model Adherence to Privacy Preferences](https://arxiv.org/abs/2507.05391)
*Guillem Ramírez,Alexandra Birch,Ivan Titov*

Main category: cs.CL

TL;DR: 本文提出通过隐私配置文件让用户控制数据隐私，构建本地模型重写查询以屏蔽敏感信息，兼顾隐私与性能，并通过多语言数据集PEEP验证方法。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型主要通过商业API访问，用户需将数据暴露给服务提供商，存在隐私风险。

Method: 设计隐私配置文件作为自然语言指令，指导本地模型重写查询，隐藏用户定义的敏感信息，再发送给外部模型。并构建PEEP数据集支持研究。

Result: 轻量级大语言模型能够在一定程度上遵守隐私指令，但仍存在理解和执行上的挑战。

Conclusion: 目前模型对用户隐私需求的理解和遵守仍有限，未来需发展更善于保护和满足用户隐私偏好的模型。
...
### [69] [Learn Globally, Speak Locally: Bridging the Gaps in Multilingual Reasoning](https://arxiv.org/abs/2507.05418)
*Jaedong Hwang,Kumar Tanmay,Seok-Jin Lee,Ayush Agrawal,Hamid Palangi,Kumar Ayush,Ila Fiete,Paul Pu Liang*

Main category: cs.CL

TL;DR: 本文提出了GeoFact-X多语言地理事实推理基准和BRIDGE训练方法，提升低资源语言的多语言推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在低资源语言的多语言推理能力不足，存在语言偏向，影响准确性和可信度，且现有基准未检测推理是否用目标语言进行。

Method: 构建包含五种语言推理痕迹的GeoFact-X基准，提出BRIDGE方法结合语言一致性奖励进行监督微调和测试时强化学习，并采用LLM作为评判者的自动评估协议。

Result: BRIDGE方法显著提升了多语言推理的语言忠实度和准确性，验证了推理感知多语言强化学习对跨语言泛化的重要性。

Conclusion: 通过语言一致性引导的多语言推理训练，可有效改善低资源语言的推理表现，推动多语言模型更好地服务于多样化语言环境。
...
### [70] ["Lost-in-the-Later": Framework for Quantifying Contextual Grounding in Large Language Models](https://arxiv.org/abs/2507.05424)
*Yufei Tao,Adam Hiatt,Rahul Seetharaman,Ameeta Agrawal*

Main category: cs.CL

TL;DR: 本文提出了CoPE评估框架，系统测量大型语言模型对上下文知识（CK）和参数知识（PK）的利用，发现了模型在处理上下文信息时存在位置偏差问题，尤其是在推理模型或链式思维提示下表现更差。通过设计基于提示的方法提升上下文利用效果，在摘要任务中显著改善事实基础和减少虚假信息。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型如何优先整合参数知识和上下文知识，以及这种整合存在的问题和改进空间。

Method: 构建多语言数据集MultiWikiAtomic，设计CoPE评估框架评测模型在开放式问答中对CK和PK的利用，分析模型的上下文信息处理偏差，提出基于提示的改进方法。

Result: 发现大型语言模型存在“lost-in-the-later”现象，后续出现的信息容易被忽视；推理模型和链式思维提示反而降低了对上下文的利用效果；基于上下文的提示法可有效提升模型事实准确性并减少虚假信息。

Conclusion: 大型语言模型在结合参数知识与上下文知识时存在较强的位置信息偏差，合理设计提示策略能促进更充分的上下文利用，提升生成文本的事实准确性和可靠性。
...
### [71] [Gendered Divides in Online Discussions about Reproductive Rights](https://arxiv.org/abs/2507.05443)
*Ashwin Rao,Sze Yuh Nina Wang,Kristina Lerman*

Main category: cs.CL

TL;DR: 本文通过分析近千万条推特上有关堕胎的帖子，探讨了性别和地方社会政治背景如何影响堕胎话语。


<details>
  <summary>Details</summary>
Motivation: 尽管对堕胎的意识形态分歧广为人知，但性别与地方社会政治背景如何交互影响公众话语这一点尚缺乏研究。

Method: 利用带有性别、意识形态和地理信息推特用户的约千万条堕胎相关帖子数据，分析性别在不同意识形态及地域背景下对堕胎态度和情绪表达的调节作用。

Result: 发现性别显著调节堕胎态度和情绪表达，保守地区该差异尤为显著，形成了随着保守程度提高而加剧的性别差距；DOBBS案草案泄露后，促进了保守区域对堕胎权威胁下支持堕胎女性的线上动员。

Conclusion: 堕胎话语不仅受意识形态极化影响，也深受性别和地域因素结构性的塑造，身份在制度性变革时刻的政治表达中发挥核心作用。
...
### [72] [PhoniTale: Phonologically Grounded Mnemonic Generation for Typologically Distant Language Pairs](https://arxiv.org/abs/2507.05444)
*Sana Kang,Myeongseok Gwon,Su Young Kwon,Jaewook Lee,Andrew Lan,Bhiksha Raj,Rita Singh*

Main category: cs.CL

TL;DR: 本文提出了PhoniTale，一种基于音韵相似性和大型语言模型生成跨语言助记词的系统，旨在帮助韩语母语者学习英语词汇。


<details>
  <summary>Details</summary>
Motivation: 二语学习中，尤其是英语与韩语等结构和语音差异较大的语言之间的词汇习得困难，现有研究多关注英语母语者学习其他语言，忽视了逆向的语言学习问题。

Method: PhoniTale系统通过检索母语关键词序列的音韵相似性，结合大型语言模型生成助记词，并通过自动评测和人工评价与以往方法及人类编写的助记词对比。

Result: 实验结果显示，PhoniTale生成的助记词在效果上与人类编写的助记词相当，且在短期记忆测试中证明了其实用性。

Conclusion: PhoniTale为二语词汇习得提供了一种有效的跨语言助记词生成方法，但仍有提升助记词质量和生成方法的空间。
...
### [73] [On the Semantics of Large Language Models](https://arxiv.org/abs/2507.05448)
*Martin Schuele*

Main category: cs.CL

TL;DR: 本文探讨大型语言模型（LLMs）如ChatGPT在词语和句子层面上的语义理解能力，通过分析其内部机制并结合经典语义理论，评估其对语言的真实理解程度。


<details>
  <summary>Details</summary>
Motivation: 目前对LLMs是否真正理解语言存在争议，本文旨在明确这一问题的语义层面细节。

Method: 通过分析LLMs的内部表征机制，结合Frege与Russell的经典语义理论，详细探讨LLMs的语义表现。

Result: 获得了LLMs在语义能力上的更加细致和深入的理解，揭示其潜在的语义处理能力。

Conclusion: LLMs在一定程度上具备语义理解能力，但对于其真正理解语言的深度仍需更细致的研究。
...
### [74] [ModelCitizens:Representing Community Voices in Online Safety](https://arxiv.org/abs/2507.05455)
*Ashima Suvarna,Christina Chance,Hamid Palangi,Sophie Hao,Thomas Hartvigsen,Saadia Gabriel*

Main category: cs.CL

TL;DR: 本文提出了一个名为MODELCITIZENS的新数据集，用于提升网络有害语言检测的多样性和社区相关性，并基于该数据集训练了性能优于现有模型的检测模型。


<details>
  <summary>Details</summary>
Motivation: 当前有害语言检测模型忽视了不同社区视角，往往将多元注释合并为单一标准，忽略了语境和被反讽使用的语言等因素，导致模型表现不佳。

Method: 构建包含6.8K社交媒体帖子和40K来自不同身份群体的有害语言注释的MODELCITIZENS数据集，并通过大语言模型生成对话场景增强数据，训练了两种基于LLaMA和Gemma的微调模型。

Result: 现有的有害语言检测工具在MODELCITIZENS数据集上表现较差，且在增加对话场景后效果进一步下降；训练的LLAMACITIZEN-8B和GEMMACITIZEN-12B模型在原数据评测中比GPT-o4-mini提高了5.5%。

Conclusion: 社区驱动的多样性注释和对话语境增强对提升有害语言自动检测模型的包容性和准确性至关重要。
...
### [75] [Empowering Healthcare Practitioners with Language Models: Structuring Speech Transcripts in Two Real-World Clinical Applications](https://arxiv.org/abs/2507.05517)
*Jean-Philippe Corbeil,Asma Ben Abacha,George Michalopoulos,Phillip Swazinna,Miguel Del-Agua,Jerome Tremblay,Akila Jeeson Daniel,Cari Bader,Kevin Cho,Pooja Krishnan,Nathan Bodenstab,Thomas Lin,Wenxuan Teng,Francois Beaulieu,Paul Vozila*

Main category: cs.CL

TL;DR: 该论文探讨了大型语言模型在临床自然语言处理中的应用，聚焦护士口述报告的结构化和医生-患者咨询中的医疗订单提取两大任务。


<details>
  <summary>Details</summary>
Motivation: 目前这两项高影响力的临床任务因数据稀缺和隐私敏感性而研究不足，但解决它们能有效减轻医护人员的文档工作量。

Method: 利用私有和开源临床数据集，评估开放权重和闭源权重的大型语言模型表现，分析其优势与不足，提出一种智能流水线生成现实但无敏感信息的护士口述报告以促进结构化临床观察提取。

Result: 发布了首个开源护士观察提取和医疗订单提取数据集SYNUR和SIMORD，推动该领域进一步研究。

Conclusion: 提供了实际可用的解决方案和数据资源，有助于改善临床文档处理效率，促进相关自然语言处理技术的发展。
...
### [76] [Enhancing Test-Time Scaling of Large Language Models with Hierarchical Retrieval-Augmented MCTS](https://arxiv.org/abs/2507.05557)
*Alex ZH Dou,Zhongwei Wan,Dongfei Cui,Xin Wang,Jing Xiong,Haokun Lin,Chaofan Tao,Shen Yan,Mi Zhang*

Main category: cs.CL

TL;DR: 本文提出了R2-LLMs，一种基于层次检索增强推理的框架，在推理时利用双层检索学习优化大语言模型表现，无需从更先进模型蒸馏训练数据。


<details>
  <summary>Details</summary>
Motivation: 当前测试时扩展语言模型以提升性能的方法需要从更高级模型蒸馏链式思维训练数据，存在成本和数据匮乏问题，因此需要一种无需蒸馏且有效提升推理能力的新方法。

Method: R2-LLMs结合粗层次抽象模板和问题答案对的检索进行高层次学习，细层次则通过蒙特卡洛树搜索检索类似的中间解步骤，并利用过程奖励模型优化每一步生成和决策。

Result: 在MATH500、GSM8K和OlympiadBench-TO数据集上，基于LLaMA-3.1-8B模型，R2-LLMs相较于基线方法最高提升了16%的性能。

Conclusion: R2-LLMs通过层次化检索及奖励模型机制，显著提升了大语言模型在复杂推理任务中的推理精度与泛化能力，展示了测试时扩展推理的潜力和有效性。
...
### [77] [Self-Review Framework for Enhancing Instruction Following Capability of LLM](https://arxiv.org/abs/2507.05598)
*Sihyun Park*

Main category: cs.CL

TL;DR: 本文提出了Re5框架，通过自我评估和有选择的修订，有效提升大型语言模型的指令遵循能力，同时保证生成内容质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然利用强大模型的数据提升大语言模型的指令遵循性，但存在高成本和过度修订导致质量下降的问题。

Method: Re5框架分解用户指令的任务和约束，进行结构化评估，精细化约束特定内容评估，并进行有选择的修订，最终利用高质量输出进行调优，实现长效迭代优化。

Result: 实验证明Re5在保持高质量回应的同时，在指令遵循性能上达到与GPT-4o-mini生成数据训练模型相当的水平，且修订后的回应在质量上优于未修订回应，胜率达64.24%。

Conclusion: Re5是一个高效且有效的解决方案，能够以极低的外部监督成本显著提升语言模型的指令遵循性和生成质量。
...
### [78] [Flipping Knowledge Distillation: Leveraging Small Models' Expertise to Enhance LLMs in Text Matching](https://arxiv.org/abs/2507.05617)
*Mingzhe Li,Jing Xiang,Qishen Zhang,Kaiyang Wan,Xiuying Chen*

Main category: cs.CL

TL;DR: 提出了一种反向知识蒸馏范式，让大型语言模型从小型模型学习，提升了领域特定任务的表现。


<details>
  <summary>Details</summary>
Motivation: 小型模型在特定领域任务如文本匹配中表现优异，但大型语言模型具备丰富语义理解，如何结合两者优势是本研究的出发点。

Method: 采用反向知识蒸馏，使用LoRA使解码器式LLM兼具编码器解码器结构，提出Margin-aware对比学习对齐相似度分数，实现准确并自适应的对比学习。

Result: 在金融、医疗等领域基准测试及真实应用中，反向蒸馏范式显著提升LLM性能，并已完成线上部署。

Conclusion: 反向知识蒸馏有效弥合大模型与小模型结构差异，充分利用小模型的领域优势，提升了大型模型在特定任务中的表现。
...
### [79] [SARA: Selective and Adaptive Retrieval-augmented Generation with Context Compression](https://arxiv.org/abs/2507.05633)
*Yiqiao Jin,Kartik Sharma,Vineeth Rakesh,Yingtong Dou,Menghai Pan,Mahashweta Das,Srijan Kumar*

Main category: cs.CL

TL;DR: 本文提出了一种名为SARA的统一检索增强生成框架，旨在解决大语言模型使用外部知识时受上下文长度限制和检索文档冗余的问题，通过结合文本摘要和语义压缩向量提升上下文效率和回答准确性。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法受限于有效上下文长度且文档存在冗余，纯压缩方法虽能减少输入大小但常丢失事实细节，影响准确性，亟需一种兼顾细节和全局知识覆盖的方案。

Method: SARA以细粒度文本片段和紧凑可解释的语义压缩向量两种互补层次表示上下文，结合迭代证据选择模块利用压缩向量动态重排上下文，实现文本与压缩向量的联合优化。

Result: 在9个数据集和5个开源大模型（Mistral、Llama、Gemma）上，SARA在回答相关性、准确性和语义相似性上均显著提升，分别提高了17.71、13.72和15.53。

Conclusion: 通过集成文本和压缩表示，SARA实现了在紧张上下文预算下同时提升检索增强生成的鲁棒性和效率，验证了该方法的重要性与有效性。
...
### [80] [ECom-Bench: Can LLM Agent Resolve Real-World E-commerce Customer Support Issues?](https://arxiv.org/abs/2507.05639)
*Haoxin Wang,Xianhan Peng,Xucheng Huang,Yizhe Huang,Ming Gong,Chenghan Yang,Yang Liu,Ling Jiang*

Main category: cs.CL

TL;DR: 本文提出了ECom-Bench，一个用于评估具备多模态能力的大型语言模型(LLM)在电商客服领域表现的基准框架。


<details>
  <summary>Details</summary>
Motivation: 电商客服场景复杂且现实，现有评测工具不足以真实反映模型的实际能力，因此需要一个更贴近真实业务环境的评测基准。

Method: ECom-Bench通过基于真实用户画像的动态用户模拟和真实电商对话数据构建任务集合，覆盖多种业务场景，模拟真实复杂的电商问题。

Result: 即使是先进的模型如GPT-4o，在该基准上的通过率也仅有10-20%，显示了评测任务的挑战性和电商客服场景的复杂性。

Conclusion: ECom-Bench为多模态LLM在电商客户支持中的性能评估提供了逼真且具有挑战性的环境，未来将开源代码和数据，推动该领域研究进展。
...
### [81] [Smoothie-Qwen: Post-Hoc Smoothing to Reduce Language Bias in Multilingual LLMs](https://arxiv.org/abs/2507.05686)
*SeungWon Ji,Jungyup Lee,Jemin Kim,Sang Park,SeungJae Lee*

Main category: cs.CL

TL;DR: 本文提出了一种名为Smoothie-Qwen的轻量级后处理方法，有效缓解了多语言大语言模型的语言混淆问题，显著减少了不期望的语言输出，同时保持任务准确率。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型往往出现语言混淆，导致模型在不同语言提示下仍产生主要语言的回复，影响多语言应用的可靠性。

Method: Smoothie-Qwen通过选择性调整输出的token级概率来抑制不期望的语言生成，无需重新训练模型。

Result: 应用于Qwen模型后，不期望的中文输出减少了95%以上，同时在多语言基准测试中的任务准确率得以保持。

Conclusion: Smoothie-Qwen为提升多语言大模型语言可控性提供了一种高效实用的解决方案，增强了其全球应用的可靠性。
...
### [82] [Agentic-R1: Distilled Dual-Strategy Reasoning](https://arxiv.org/abs/2507.05707)
*Weihua Du,Pranjal Aggarwal,Sean Welleck,Yiming Yang*

Main category: cs.CL

TL;DR: 提出了一种名为DualDistill的微调框架，通过从多个教师模型中提炼互补的推理策略，训练出能动态选择最优策略的Agentic-R1，提升了数学和逻辑推理任务的准确率和效率。


<details>
  <summary>Details</summary>
Motivation: 当前长链思维模型在数学推理上表现良好但依赖易错且缓慢的自然语言轨迹，工具增强代理虽然通过代码执行解决算术问题，但在复杂逻辑任务中表现不佳。需要一种方法整合多种推理策略以提高推理的鲁棒性和效率。

Method: 提出DualDistill微调框架，将多种教师模型的推理策略蒸馏到统一的学生模型中，训练Agentic-R1动态选择并调用适当的工具（代码执行或文本推理）解决不同类型的问题。

Result: Agentic-R1在计算密集型和标准基准测试中均表现出准确率提升，证明了多策略蒸馏方法在实现高效且鲁棒推理上的有效性。

Conclusion: 通过多策略蒸馏，Agentic-R1能够灵活选择最优推理方式，显著提升了复杂数学和逻辑任务的推理性能与效率。该方法为提升人工智能系统的推理能力提供了一种有效途径。
...
### [83] [DRAGON: Dynamic RAG Benchmark On News](https://arxiv.org/abs/2507.05713)
*Fedor Chernogorskii,Sergei Averkiev,Liliya Kudraleeva,Zaven Martirosian,Maria Tikhonova,Valentin Malykh,Alena Fenogenova*

Main category: cs.CL

TL;DR: 本文提出了一个名为DRAGON的动态基准，用于评估俄语检索增强生成（RAG）系统，基于不断更新的新闻语料库，支持检索器和生成器的全面评测。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成（RAG）基准多为英语，俄语等其他语言资源匮乏且静态，难以反映真实世界中动态变化的需求。

Method: 构建一个动态更新的俄语新闻和公共文档语料库，自动生成基于知识图的多种类型问题，支持检索组件和生成组件的综合评估。

Result: 提供自动问题生成管线、评测脚本和基准数据，支持多语言和多语言环境使用，同时发布公开排行榜促进行业交流。

Conclusion: DRAGON填补了俄语RAG动态评估的空白，促进基于新闻动态变化的知识增强生成系统的研究与比较。
...
### [84] [HIRAG: Hierarchical-Thought Instruction-Tuning Retrieval-Augmented Generation](https://arxiv.org/abs/2507.05714)
*YiHan Jiao,ZheHao Tan,Dan Yang,DuoLin Sun,Jie Feng,Jian Wang,Peng Wei*

Main category: cs.CL

TL;DR: 本文提出了一种新方法HIRAG，通过层次化链式思考指令微调提升检索增强生成模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前RAG模型在处理实时信息和领域特定问题时面临文档质量不一致和检索系统缺陷，且缺乏针对RAG任务的细粒度研究和链式思考的深度利用。

Method: 提出RAG模型应具备过滤、组合和特定推理三层能力，设计层次化思考指令微调方法HIRAG，采用“先思考后回答”策略，利用多级链式思考增强模型的开放式考试能力。

Result: HIRAG训练策略在多个数据集（RGB、PopQA、MuSiQue、HotpotQA、PubmedQA）上显著提升模型性能。

Conclusion: HIRAG有效改进了RAG模型的生成能力，提升了模型对外部知识和内部知识的结合与推理水平。
...
### [85] [Omni-Router: Sharing Routing Decisions in Sparse Mixture-of-Experts for Speech Recognition](https://arxiv.org/abs/2507.05724)
*Zijin Gu,Tatiana Likhomanenko,Navdeep Jaitly*

Main category: cs.CL

TL;DR: 本文提出了Omni-router Transformer，通过共享路由器提升多层专家模型间的合作与专项化，显著降低ASR任务中的错误率。


<details>
  <summary>Details</summary>
Motivation: 传统多专家模型中各层路由器的选择缺乏相关性，限制了专家间的合作和专项化效果。

Method: 采用跨层共享的路由器设计，即Omni-router Transformer，促进不同专家层间的合作与专项化。

Result: 在大规模伪标记数据和10个多样化的无监督ASR基准测试上，Omni-router Transformer训练损失更低，平均词错误率分别降低11.2%和8.2%，优于Dense和Switch Transformer。

Conclusion: 共享路由器的设计有效提高了多层专家模型的性能和鲁棒性，增强了模型在多样化数据上的表现和专家使用结构性。
...
### [86] [GPTKB v1.5: A Massive Knowledge Base for Exploring Factual LLM Knowledge](https://arxiv.org/abs/2507.05740)
*Yujia Hu,Tuan-Phong Nguyen,Shrestha Ghosh,Moritz Müller,Simon Razniewski*

Main category: cs.CL

TL;DR: 本文介绍了GPTKB v1.5，一个包含1亿三元组的大型知识库，基于GPT-4.1构建，支持链路遍历、SPARQL查询和知识比较。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型的事实知识理解有限，且难以进行灵活浏览和大规模统计分析。

Method: 采用大规模递归语言模型知识物化（GPTKB方法），以GPT-4.1自动构建密集关联的知识库。

Result: 成功构建了成本约14000美元的1亿三元组知识库，支持多样化的知识探索和查询方式。

Conclusion: 大规模递归语言模型知识物化为系统化分析和自动构建知识库提供了重要机遇，推动LLM知识的深入理解与应用。
...
### [87] [DocTalk: Scalable Graph-based Dialogue Synthesis for Enhancing LLM Conversational Capabilities](https://arxiv.org/abs/2507.05750)
*Jing Yang Lee,Hamed Bonab,Nasser Zalmout,Ming Zeng,Sanket Lokegaonkar,Colin Lockard,Binxuan Huang,Ritesh Sarkhel,Haodong Wang*

Main category: cs.CL

TL;DR: 本文提出一种从文本语料合成多轮对话数据的方法，用于增强大语言模型在多轮会话任务中的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型预训练数据主要是连续散文，与多轮对话任务需求存在差距，限制了其多轮对话能力的发展。

Method: 设计一个管道，将多篇相关文档转化为包含多轮多话题的信息查询对话，并在维基百科文章上构建了包含73万多长对话的DocTalk语料库。

Result: 在预训练过程中引入DocTalk后，模型的上下文记忆和理解能力提升了最高达40%，且基础性能未受影响。

Conclusion: 通过合成对话结构的预训练数据，可以有效提升大语言模型的多轮对话能力，DocTalk语料已公开发布。
...
### [88] [Flippi: End To End GenAI Assistant for E-Commerce](https://arxiv.org/abs/2507.05788)
*Anand A. Rajasekar,Praveen Tangarajan,Anjali Nainani,Amogh Batwal,Vinay Rao Dandin,Anusua Trivedi,Ozan Ersoy*

Main category: cs.CL

TL;DR: 本文介绍了Flippi，一款基于大型语言模型的电子商务对话助手，通过自然语言对话提升用户产品发现效率和个性化购物体验。


<details>
  <summary>Details</summary>
Motivation: 面对庞大复杂的产品信息，传统搜索难以满足用户个性化需求，亟需更高效、智能的购物助手。

Method: Flippi结合查询重构、意图识别、检索增强生成、命名实体识别和上下文简化等先进NLP技术，解析用户需求并提供精准产品信息。

Result: Flippi能识别最优惠报价，支持产品比较，帮助用户做出更经济和明智的购物决策，提升用户参与度和转化率。

Conclusion: Flippi通过自然语言交互打造个性化购物体验，实现线上购物便利性与实体店个性化服务的结合，树立了数字市场客户满意度新标杆。
...
### [89] [Bridging Perception and Language: A Systematic Benchmark for LVLMs' Understanding of Amodal Completion Reports](https://arxiv.org/abs/2507.05799)
*Amane Watahiki,Tomoki Doi,Taiga Shinozaki,Satoshi Nishida,Takuya Niikawa,Katsunori Miyahara,Hitomi Yanaka*

Main category: cs.CL

TL;DR: 本文构建了一个基于基础形式本体的视觉语言模型（LVLM）无形补全任务基准，评估模型对隐藏物体推理能力，发现部分模型在日本语提示下表现较弱。


<details>
  <summary>Details</summary>
Motivation: 当前大多数研究关注视觉模型对遮挡区域的检测和重建，但缺乏对视觉语言模型在处理与无形补全相关文本推理能力的系统探讨。

Method: 构建了一个基于基础形式本体的无形补全任务基准，用于系统性分类和评估多模态视觉语言模型对描述遮挡物体的推理能力。

Result: 多数LVLM整体表现接近人类水平，但在特定对象类别的无形补全任务中准确率存在差异。特别是部分LLaVA-NeXT变体和Claude 3.5 Sonnet在日本语提示下，对原图表现低于无视觉内容的空白刺激。

Conclusion: LVLM在无形补全推理任务中虽整体表现良好，但在特定语言环境（如日语）存在语言能力不足的问题，需加强多语言理解能力。
...
### [90] [How to Evaluate Automatic Speech Recognition: Comparing Different Performance and Bias Measures](https://arxiv.org/abs/2507.05885)
*Tanvina Patel,Wiebke Hutiri,Aaron Yi Ding,Odette Scharenborg*

Main category: cs.CL

TL;DR: 本文比较了多种性能和偏差度量方法，评估用于荷兰语的先进端到端自动语音识别系统，发现平均错误率不足以全面反映偏差，提出了改进报告的建议。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在性别、年龄、口音等方面存在偏差，现有研究多关注检测和缓解偏差，但如何度量系统的性能和偏差仍是未解问题。

Method: 比较文献中和新提出的多种性能及偏差度量方法，在多种偏差缓解策略下对荷兰语端到端ASR系统进行评估。

Result: 发现平均错误率这一ASR标准指标不足以充分反映系统偏差，建议结合其他度量指标更全面地评估系统性能和偏差。

Conclusion: 建议在报告ASR系统性能时，综合使用多种度量指标，以更好地反映系统在不同说话人群体中的表现及整体偏差。
...
### [91] [Psychometric Item Validation Using Virtual Respondents with Trait-Response Mediators](https://arxiv.org/abs/2507.05890)
*Sungjib Lim,Woojung Song,Eun-Ju Lee,Yohan Jo*

Main category: cs.CL

TL;DR: 本文提出了一种使用大型语言模型（LLMs）模拟虚拟回答者以高效生成和验证心理测量调查题目的框架，解决了传统方法中构念效度验证成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在心理测量调查中的应用增多，如何规模化生成具备构念效度的调查题目成为挑战，传统依赖大规模人工数据成本高且效率低。

Method: 提出通过模拟具有多样中介变量的虚拟回答者，利用LLMs生成可能的中介变量并模拟回答行为，从而筛选出稳健测量目标特质的高效题目。

Result: 在大五人格、施瓦茨价值观和VIA美德三种心理特质理论上的实验表明，该框架和中介生成方法有效识别出了高效有效的题目。LLMs能够基于特质定义生成可信的中介变量并模拟回答者行为验证题目。

Conclusion: 该研究提出的方法降低了设计和验证心理调查题目的成本，推动了心理测量高效发展，并深化了对LLMs模拟人类行为能力的理解。数据集和代码将公开以促进后续研究。
...
### [92] [Few-shot text-based emotion detection](https://arxiv.org/abs/2507.05918)
*Teodor-George Marchitan,Claudiu Creanga,Liviu P. Dinu*

Main category: cs.CL

TL;DR: 本文介绍了Unibuc-NLP团队在SemEval 2025第11任务中使用大型语言模型进行多标签情感检测的方案，取得了不同子集的F1-macro成绩。


<details>
  <summary>Details</summary>
Motivation: 解决基于文本的情感检测中的多语言、多标签识别问题。

Method: 采用大型语言模型（Gemini、Qwen、DeepSeek）进行少样本提示或微调。

Result: 英文子集F1-macro为0.7546，葡萄牙语子集为0.1727，Emakhuwa子集为0.325，并在Emakhuwa子集排名第一。

Conclusion: 基于大型语言模型的微调和少样本提示方法在多标签情感检测任务中表现良好，尤其在低资源语言Emakhuwa上效果突出。
...
### [93] [Towards a Principled Evaluation of Knowledge Editors](https://arxiv.org/abs/2507.05937)
*Sebastian Pohl,Max Ploner,Alan Akbik*

Main category: cs.CL

TL;DR: 本论文探讨了知识编辑领域评估方法的鲁棒性及其可能的偏向性，同时关注编辑操作对模型整体能力的影响。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑评估数据集采用不同的评分方法，但这些方法的稳定性和公平性尚未得到充分研究，且编辑对模型整体性能的潜在影响常被忽视。

Method: 分析不同评估指标、方法及编辑批量大小对知识编辑器排名的影响，结合通用语言理解任务和手动评估字符串匹配方法的准确性。

Result: 发现评估方法和编辑批量大小会导致知识编辑器排名差异，同时字符串匹配评估存在较多误报，且编辑对模型语言理解能力有显著影响。

Conclusion: 评估方法和参数选择会显著影响知识编辑评估结果，当前流行的字符串匹配方法存在不足，强调需全面考察编辑对模型能力的影响。
...
### [94] [Remember Past, Anticipate Future: Learning Continual Multimodal Misinformation Detectors](https://arxiv.org/abs/2507.05939)
*Bing Wang,Ximing Li,Mengzhe Ye,Changchun Li,Bo Fu,Jianfeng Qu,Lin Yuanbo Wu*

Main category: cs.CL

TL;DR: 本文提出了一种持续性多模态谣言检测方法DAEDCMD，以应对不断出现的新事件导致模型过时的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态谣言检测模型在现实场景面临新事件不断出现导致的模型遗忘和环境变化影响泛化的问题。

Method: 通过基于Dirichlet过程的专家混合结构隔离事件特定参数以记忆过去知识，同时学习连续时间动力学模型预测未来环境分布，提出持续检测方法DAEDCMD。

Result: DAEDCMD在多模态谣言检测任务中，显著优于六个基础方法和三个持续学习方法。

Conclusion: DAEDCMD有效缓解了知识遗忘和环境演变问题，提高了模型在持续数据流上的谣言检测性能。
...
### [95] [Chat-Ghosting: A Comparative Study of Methods for Auto-Completion in Dialog Systems](https://arxiv.org/abs/2507.05940)
*Sandeep Mishra,Anubhab Mandal,Bishal Santra,Tushar Abhishek,Pawan Goyal,Manish Gupta*

Main category: cs.CL

TL;DR: 该论文研究了聊天系统中的文本输入预测（Ghosting）问题，比较了多种方法并提出了一种基于熵的动态早停策略，结果显示统计模型在已见前缀上表现优于深度学习模型，而深度学习模型在未见查询上更优，加入对话上下文显著提升预测质量。


<details>
  <summary>Details</summary>
Motivation: 随着聊天系统（如ChatGPT、Copilot）普及，预测用户意图文本输入（Ghosting）成为提升用户体验的重要挑战，但此问题在NLP/ML领域研究不足，缺乏标准基准和性能分析。

Method: 采用四个公开对话数据集（人-人和人-机），对比基于trie、n-gram和深度学习模型（含T5、Phi-2），并加入对话上下文，提出基于熵的动态早停策略。

Result: 统计n-gram和trie模型在已见前缀上性能和推理效率优于深度学习模型；深度学习模型（T5、Phi-2）在未见查询上表现更好；对话上下文的加入显著提升Ghosting质量。

Conclusion: 研究揭示了不同方法在Ghosting任务上的优劣，强调对话上下文的重要性，并提供开源代码和数据，为该领域后续研究提供了重要资源和基准。
...
### [96] [OpenFActScore: Open-Source Atomic Evaluation of Factuality in Text Generation](https://arxiv.org/abs/2507.05965)
*Lucas Fonseca Lage,Simon Ostermann*

Main category: cs.CL

TL;DR: OpenFActScore是FActScore框架的开源实现，用于评估大型语言模型生成文本的真实性，通过原子事实生成和验证方法，支持任何兼容Hugging Face的模型，表现接近闭源系统。


<details>
  <summary>Details</summary>
Motivation: 原FActScore依赖闭源商业模型，限制了透明性和可重复性，迫切需要一个开源、低成本且可替代的工具。

Method: 利用原子事实生成(AF G)提取文本中的单个事实主张，利用原子事实验证(AF V)将其与可信知识源校验，支持多种开源LLM实现该流程，并在原FActScore基准上评测性能。

Result: 多种开源LLM表现接近闭源商业模型，其中Gemma模型效果最好，最终实现与原FActScore实验0.99的Pearson相关性。

Conclusion: OpenFActScore提升了事实性评估的透明性、可重复性和成本效益，是一个可靠的开源工具，推动相关研究的发展。
...
### [97] [We Should Evaluate Real-World Impact](https://arxiv.org/abs/2507.05973)
*Ehud Reiter*

Main category: cs.CL

TL;DR: ACL领域极少关注NLP系统的实际影响评估，相关研究仅占0.1%，且大多评估不深入。


<details>
  <summary>Details</summary>
Motivation: 当前ACL社区对NLP系统在现实世界中的影响缺乏重视，评估工作稀少且不充分，阻碍了技术的有效应用和推广。

Method: 通过对ACL Anthology文献的系统性调查，统计了包含影响评估论文的比例及其评估质量。

Result: 只有极少数论文（约0.1%）涉及NLP系统的实际影响评估，且多以指标评估为主，缺乏深入和系统的影响分析。

Conclusion: 应加强对NLP系统现实影响的理解和评估，这将促进技术的实用性提升和更快的实际应用采纳。
...
### [98] [RabakBench: Scaling Human Annotations to Construct Localized Multilingual Safety Benchmarks for Low-Resource Languages](https://arxiv.org/abs/2507.05980)
*Gabriel Chua,Leanne Tan,Ziyu Ge,Roy Ka-Wei Lee*

Main category: cs.CL

TL;DR: 本文介绍了RabakBench，一个针对新加坡多语种（包括新加坡式英语、中文、马来语和泰米尔语）的安全性基准测试数据集，旨在改善大型语言模型对低资源语言的安全分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型及其安全分类器在低资源语言上表现不佳，缺少合适的训练数据和评估基准，尤其是在新加坡特殊的多语言环境中。

Method: 采用三阶段流水线：生成（利用大型语言模型对真实新加坡式英语内容进行对抗样本生成）、标注（结合多个语言模型和人类判断的半自动多标签安全标注）、翻译（高保真翻译，保持语言细微差别和毒性），最终形成覆盖四种语言、六个细粒度安全类别的数据集。

Result: 构建了包含5000多个带安全标签示例的数据集，评测11种主流开源和闭源安全分类器，发现其性能显著下降，体现现有模型在此类多语言环境下的不足。

Conclusion: RabakBench不仅为东南亚多语言安全评估提供了坚实基础，还提出了在低资源环境构建本土化安全数据集的可复制框架，数据集和代码已公开。
...
### [99] [Evolution without Large Models: Training Language Model with Task Principles](https://arxiv.org/abs/2507.05991)
*Minghang Zhu,Shen Gao,Zhengliang Shi,Jiabao Fang,Pengjie Ren,Zhaochun Ren,Zhumin Chen,Shuo Shang*

Main category: cs.CL

TL;DR: 本文提出了一种语言模型自我进化方法，通过多层次原则生成和基于原则的实例生成，利用大模型总结任务原则并由小模型生成数据，从而降低碳排放并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前使用大规模语言模型扩充数据集的训练方法虽然降低了人工标注成本，但存在高碳排放和数据泄露风险问题。

Method: 提出多层次原则生成方法让大模型总结任务完成原则，随后小模型基于这些原则生成大量训练数据。

Result: 实验表明该方法比直接用小模型生成数据显著提升模型性能，同时大大减少了碳排放。

Conclusion: 该方法有效解决了现有数据扩充中的碳排放和数据风险问题，提升了训练效率和效果。
...
### [100] [DocIE@XLLM25: In-Context Learning for Information Extraction using Fully Synthetic Demonstrations](https://arxiv.org/abs/2507.05997)
*Nicholas Popovič,Ashish Kangen,Tim Schopf,Michael Färber*

Main category: cs.CL

TL;DR: 本文提出了一种基于大型语言模型的自动化合成数据生成和上下文学习管道，用于文档级实体和关系抽取，在零样本和少样本条件下提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有文档级实体和关系抽取任务中，高质量标注语料稀缺，且传统方法依赖手动标注示例或直接零样本推理效果有限。

Method: 采用推理优化的大型语言模型，结合自动合成数据生成和基于检索的上下文学习，构建高质量示例数据库，并动态检索相关示例进行推理。

Result: 生成了一个包含5000多个维基百科摘要的合成数据集，包含约59000个实体和30000个关系三元组。在DocIE共享任务的零样本环境下进行评测，验证了方法有效性。

Conclusion: 即使是最先进的大型语言模型，在文档级的联合实体和关系抽取中仍面临挑战，显示该任务难度较大。
...
### [101] [Conditional Multi-Stage Failure Recovery for Embodied Agents](https://arxiv.org/abs/2507.06016)
*Youmna Farag,Svetlana Stoyanchev,Mohan Li,Simon Keizer,Rama Doddipatla*

Main category: cs.CL

TL;DR: 本文提出了一种基于零样本链式提示的多阶段故障恢复框架，用于提升具身智能体执行复杂任务时的失败恢复能力，方法在TEACH数据集上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 具身智能体在执行复杂任务时容易出现执行失败，需要有效的故障恢复机制以提高任务完成率。

Method: 引入一个条件多阶段故障恢复框架，包含四个错误处理阶段（任务执行中三个阶段和一个后执行反思阶段），利用大型语言模型(LLMs)的推理能力结合环境上下文分析执行中的问题并制定策略解决方案。

Result: 在TEACH数据集的TfD基准测试中，提出方法比不采用故障恢复的基线方法提升了11.5%，并且比最强的现有模型提升了19%。

Conclusion: 该多阶段故障恢复框架有效提升了具身智能体的任务执行能力，证明了利用LLMs进行故障分析和策略制定的有效性。
...
### [102] [Entropy-Memorization Law: Evaluating Memorization Difficulty of Data in LLMs](https://arxiv.org/abs/2507.06056)
*Yizhan Huang,Zhe Yang,Meifang Chen,Jianping Zhang,Michael R. Lyu*

Main category: cs.CL

TL;DR: 本文探讨了大语言模型中训练数据记忆难度的特性，提出了熵-记忆法则，并基于此实现了训练与测试数据的区分方法，即数据集推断。


<details>
  <summary>Details</summary>
Motivation: 大语言模型会记忆部分训练数据，但如何定量刻画训练数据的记忆难度尚未充分研究。

Method: 通过实验证明数据熵与记忆得分线性相关，研究了随机字符串的熵值特性，并基于熵-记忆法则开发数据集推断方法。

Result: 发现随机字符串的经验熵低于整体语料，验证了熵-记忆法则的有效性，并成功区分训练与测试数据。

Conclusion: 熵是衡量训练数据记忆难度的有效指标，基于此可以实现训练数据的识别与区分。
...
### [103] [A Survey on Prompt Tuning](https://arxiv.org/abs/2507.06085)
*Zongqian Li,Yixuan Su,Nigel Collier*

Main category: cs.CL

TL;DR: 本文综述了通过在语言模型前添加可训练的连续向量以实现参数高效调整的提示调优方法，分为直接提示学习和迁移学习两大类。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型参数量增长，传统微调方法计算成本高，提示调优作为参数高效方法应运而生。

Method: 文章分类总结了直接提示学习（包含通用优化、编码器方法、分解策略和专家混合框架）和迁移学习（包含通用迁移、编码器方法、分解策略）的方法，分析了各方法的设计、创新、优缺点，并配以比较可视化。

Result: 识别出提示调优在计算效率和训练稳定性方面的挑战，提供了方法对比和技术解析。

Conclusion: 文章指出未来应提升训练鲁棒性，拓展应用范围，以促进提示调优技术的发展。
...
### [104] [NeoBabel: A Multilingual Open Tower for Visual Generation](https://arxiv.org/abs/2507.06137)
*Mohammad Mahdi Derakhshani,Dheeraj Varghese,Marzieh Fadaee,Cees G. M. Snoek*

Main category: cs.CL

TL;DR: NeoBabel是一种支持六种语言的多语言文本生成图像框架，在多语言性能、效率和包容性方面表现优异，超过现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成图像系统主要以英语为中心，给非英语用户带来障碍，并导致文化不匹配和计算负担。

Method: 采用大规模多语言预训练和高分辨率指令调优，训练出支持英语、中文、荷兰语、法语、印地语和波斯语的NeoBabel模型，同时扩展英文基准数据集为多语言版本。

Result: NeoBabel在多语言评测中表现出色，超过现有多语言大模型，且模型体积更小，效率更高，其多语言能力提升了鲁棒性和文化适应性。

Conclusion: 多语言能力不仅不会降低模型性能，反而促进生成AI的鲁棒性、效率和文化忠实度，NeoBabel推动了包容性AI研究的发展。
...
### [105] [Coding Triangle: How Does Large Language Model Understand Code?](https://arxiv.org/abs/2507.06138)
*Taolin Zhang,Zihan Ma,Maosong Cao,Junnan Liu,Songyang Zhang,Kai Chen*

Main category: cs.CL

TL;DR: 提出Code Triangle框架系统评估大语言模型在编辑分析、代码实现和测试用例生成三方面的编程能力，发现模型表现欠缺多样性和鲁棒性，且存在与人类认知的分布偏移，通过引入人类编辑、解决方案和多样测试用例以及模型混合提升性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在代码生成上取得进展，但其真实编程能力尚未充分探究，需系统评估其在多个维度的表现。

Method: 设计Code Triangle框架从编辑分析、代码实现和测试用例生成三个核心维度评估LLM，结合竞赛基准进行大量实验，分析模型认知偏差和错误模式，并探索引入人类数据及模型混合策略提升性能。

Result: LLM的解决方案虽自洽，但缺乏人类的多样性和鲁棒性，模型错误集群现象明显，模型认知与人类专业知识存在显著分布迁移，利用人类数据和模型混合有效提升了性能和鲁棒性。

Conclusion: Code Triangle框架展示了LLM在编程认知上的一致性与不一致性，有助于自我反思和提升，指明未来构建更强大代码模型的方向。
...
### [106] [Skywork-R1V3 Technical Report](https://arxiv.org/abs/2507.06167)
*Wei Shen,Jiangbo Pei,Yi Peng,Xuchen Song,Yang Liu,Jian Peng,Haofeng Sun,Yunzhuo Hao,Peiyu Wang,Yahui Zhou*

Main category: cs.CL

TL;DR: Skywork-R1V3是一款先进的开源视觉语言模型，通过强化学习后训练框架转移文本大模型的推理能力，实现了多模态视觉推理的新突破，性能达到人类入门水平。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型推理能力有限，且难以有效利用仅文本训练的大型语言模型的推理技巧，亟需一种新方法提升视觉推理性能。

Method: 采用细致设计的后训练强化学习框架激活并增强模型的推理能力，无需额外继续预训练，重点利用连接模块实现跨模态对齐；引入推理能力指标（关键信息熵）指导训练检查点选择。

Result: Skywork-R1V3在多模态推理基准MMMU上表现显著提升，准确率从64.3%提升至76.0%，达到初级人类水平，38B参数模型性能可匹敌顶级闭源模型。

Conclusion: 通过强化学习后训练，Skywork-R1V3成功实现了从文本大模型向视觉域的推理能力迁移，显著推动开源视觉语言模型的多模态推理能力发展。
...
### [107] [CriticLean: Critic-Guided Reinforcement Learning for Mathematical Formalization](https://arxiv.org/abs/2507.06181)
*Zhongyuan Peng,Yifan Yao,Kaijing Ma,Shuyue Guo,Yizhe Li,Yichi Zhang,Chenchen Zhang,Yifan Zhang,Zhouliang Yu,Luming Li,Minghao Liu,Yihang Xia,Jiawei Shen,Yuchen Wu,Yixin Cao,Zhaoxiang Zhang,Wenhao Huang,Jiaheng Liu,Ge Zhang*

Main category: cs.CL

TL;DR: 本文提出了CriticLean框架，通过强化学习提升数学自然语言转形式化代码的语义评估准确性，构建了相关模型和数据集，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动定理证明工作主要关注生成和编译成功，忽视了评估生成的形式化表达是否准确传达原始问题的语义意图的重要性。

Method: 提出CriticLean框架，设计CriticLeanGPT模型用于语义准确度评估，结合监督微调和强化学习进行训练；构建CriticLeanBench基准测试模型区分正确与错误的形式化，最后制作涵盖领域丰富、难度广泛的FineLeanCorpus数据集。

Result: 训练后的CriticLeanGPT模型在CriticLeanBench基准上显著超过现有开源与闭源强基线方法，且FineLeanCorpus数据集经人工验证具有高正确性。

Conclusion: 优化评估阶段对于生成可靠的形式化表达至关重要，CriticLean为形式数学推理领域未来研究提供了新的思路和工具。
...
### [108] [DS@GT at CheckThat! 2025: Detecting Subjectivity via Transfer-Learning and Corrective Data Augmentation](https://arxiv.org/abs/2507.06189)
*Maximilian Heil,Dionne Bang*

Main category: cs.CL

TL;DR: 本文通过转移学习和风格化数据增强提升英文新闻文本主观性检测，转移专用编码器优于通用编码器，增强方法显著提升模型鲁棒性，比赛排名第16。


<details>
  <summary>Details</summary>
Motivation: 提升英文新闻文本中主观与客观句子的分类准确性，探索转移学习和风格数据增强的有效性。

Method: 对比预训练编码器微调与相关任务的转移学习，利用GPT-4o生成风格化释义进行受控数据增强，确保标签和风格一致性采用同模型进行修正。

Result: 转移学习专用编码器优于通用编码器微调，经过精心策划的数据增强显著提高了模型在主观内容识别上的鲁棒性，最终比赛排名16/24。

Conclusion: 结合编码器专业化与标签一致的数据增强是提升主观性检测效果的有效方法。
...
### [109] [DS@GT at CheckThat! 2025: Evaluating Context and Tokenization Strategies for Numerical Fact Verification](https://arxiv.org/abs/2507.06195)
*Maximilian Heil,Aleksandar Pramov*

Main category: cs.CL

TL;DR: 本文评估了自动事实核查中对包含数量信息的声明的真实性预测方法，使用QuanTemp数据集并搭建证据检索管道。


<details>
  <summary>Details</summary>
Motivation: 数量相关的声明在自动事实核查中具有独特的挑战，现有方法效果有限。

Method: 通过实验考察更长上下文窗口的证据、右到左(R2L)分词策略及其组合对模型性能的影响，应用ModernBERT模型。

Result: 发现R2L分词和更长上下文窗口均未提升性能，证据质量是主要瓶颈；最佳系统在CheckThat!2025任务中获得前4名，宏F1得分0.57。

Conclusion: 提高证据质量比调整输入长度或分词方向更关键，研究为数值声明的自动事实核查提供有效方向。
...
### [110] [UQLM: A Python Package for Uncertainty Quantification in Large Language Models](https://arxiv.org/abs/2507.06196)
*Dylan Bouchard,Mohit Singh Chauhan,David Skarbrevik,Ho-Kyeong Ra,Viren Bajaj,Zeya Ahmad*

Main category: cs.CL

TL;DR: 本文介绍了UQLM，一个基于不确定性量化技术的LLM幻觉检测Python工具包。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型生成虚假或误导性内容（幻觉）问题，保障下游应用的安全性与可信性。

Method: 开发了一套基于不确定性量化的评分器，计算模型响应的置信度得分（0到1）。

Result: 提供了一个开箱即用的工具库，便于集成用于提升LLM输出的可靠性。

Conclusion: 通过UQLM工具包的应用，可以有效辅助检测LLM幻觉，增强模型输出的安全性和可信度。
...
### [111] [A Survey on Latent Reasoning](https://arxiv.org/abs/2507.06203)
*Rui-Jie Zhu,Tianhao Peng,Tianhao Cheng,Xingwei Qu,Jinfa Huang,Dawei Zhu,Hao Wang,Kaiwen Xue,Xuanliang Zhang,Yong Shan,Tianle Cai,Taylor Kergan,Assel Kembay,Andrew Smith,Chenghua Lin,Binh Nguyen,Yuqi Pan,Yuhong Chou,Zefan Cai,Zhenhe Wu,Yongchi Zhao,Tianyu Liu,Jian Yang,Wangchunshu Zhou,Chujie Zheng,Chongxuan Li,Yuyin Zhou,Zhoujun Li,Zhaoxiang Zhang,Jiaheng Liu,Ge Zhang,Wenhao Huang,Jason Eshraghian*

Main category: cs.CL

TL;DR: 本文综述了隐式推理这一前沿领域，探讨了神经网络层作为推理基础的作用以及多种隐式推理方法，展示了最新研究进展和未来方向。


<details>
  <summary>Details</summary>
Motivation: 传统的链式思维（CoT）虽提升了大语言模型的推理能力，但受限于自然语言表达，限制了模型的表现力。隐式推理通过在连续隐藏状态中进行多步推理，突破了这一瓶颈。

Method: 本文首先分析神经网络层如何作为推理的计算基础，强调层次化表征的重要性；随后探讨了多种隐式推理技术，包括基于激活的递归、隐藏状态传递和微调策略，以压缩或内化显式推理轨迹；最后介绍了基于掩码扩散模型的无限深度隐式推理方法，实现了全局一致且可逆的推理过程。

Result: 本文综合梳理了隐式推理的理论基础、主要方法和最新范式，提供了对该领域的系统理解，并通过GitHub资源汇集了相关论文和代码。

Conclusion: 通过统一视角，本文澄清了隐式推理的概念框架，为推动大语言模型认知前沿的研究指明了方向。
...
### [112] [DS@GT at CheckThat! 2025: Ensemble Methods for Detection of Scientific Discourse on Social Media](https://arxiv.org/abs/2507.06205)
*Ayush Parikh,Hoang Thanh Thanh Truong,Jeanette Schofield,Maximilian Heil*

Main category: cs.CL

TL;DR: 本文介绍了DS@GT团队在CLEF 2025 CheckThat! Task 4a科学网络话语检测任务中的方法，提出了三种建模方法，并取得了较好成绩。


<details>
  <summary>Details</summary>
Motivation: 在多类别分类中识别推文是否包含科学声明、引用或科学实体，以提升科学信息检测的准确性。

Method: 提出了基于Transformer微调、少样本提示大语言模型以及基于实验设计的集成模型三种方法。

Result: 团队在竞赛中获得第7名，宏平均F1分数达到0.8611，优于DeBERTaV3基线的0.8375。

Conclusion: 结合多种模型方法能有效提升科学话语检测任务的性能，验证了集成策略的优势。
...
### [113] [Efficiency-Effectiveness Reranking FLOPs for LLM-based Rerankers](https://arxiv.org/abs/2507.06223)
*Zhiyuan Peng,Ting-ruen Wei,Tingyu Song,Yilun Zhao,Yi Fang*

Main category: cs.CL

TL;DR: 本文提出了针对大型语言模型(LLM)重排序任务效率的硬件无关度量指标E²R-FLOPs，包括每PetaFLOP相关性指标RPP和查询处理能力指标QPP，并设计了FLOPs估算器，评估了不同架构的LLM重排序器的效率与效果平衡。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM重排序效率的代理指标依赖硬件和运行参数，且未考虑模型规模，导致效率-效果权衡难以解释和比较。

Method: 提出E²R-FLOPs指标，包含RPP（相关性每计算量）和QPP（硬件无关吞吐量），并设计可解释的FLOPs估算器，实现无需运行实验即可估计FLOPs。

Result: 基于新指标，针对不同架构的LLM重排序模型进行了系统评测，揭示了效率与效果的权衡情况。

Conclusion: E²R-FLOPs指标有效促进了LLM重排序效率与效果的公平评价，有助于推动研究社区关注和改进该问题。
...
### [114] [Agent KB: Leveraging Cross-Domain Experience for Agentic Problem Solving](https://arxiv.org/abs/2507.06229)
*Xiangru Tang,Tianrui Qin,Tianhao Peng,Ziyang Zhou,Daniel Shao,Tingting Du,Xinming Wei,Peng Xia,Fang Wu,He Zhu,Ge Zhang,Jiaheng Liu,Xingyao Wang,Sirui Hong,Chenglin Wu,Hao Cheng,Chi Wang,Wangchunshu Zhou*

Main category: cs.CL

TL;DR: Agent KB通过分层经验框架和Reason-Retrieve-Refine流程提升语言智能体的复杂任务解决能力，实现跨智能体经验共享和迁移，显著提高成功率。


<details>
  <summary>Details</summary>
Motivation: 传统语言智能体在复杂任务中难以进行有效的错误纠正和跨域经验复用，且无法从其他智能体的经验中学习。

Method: 提出Agent KB框架，采用分层的经验管理与Reason-Retrieve-Refine流程，捕捉高阶策略与详细执行日志，构建共享知识库，实现跨智能体知识迁移。

Result: 在GAIA基准测试中，Agent KB使Claude-3和GPT-4在复杂及中级任务的成功率分别提升约16个百分点以上；在SWE-bench代码修复任务中，Claude-3成功率提升至53.33%。

Conclusion: Agent KB提供了一个模块化且框架无关的基础设施，支持语言智能体从过去经验中学习并将成功策略推广应用到新任务。
...
### [115] [CORE: Benchmarking LLMs Code Reasoning Capabilities through Static Analysis Tasks](https://arxiv.org/abs/2507.05269)
...
### [116] [Open Source, Hidden Costs: A Systematic Literature Review on OSS License Management](https://arxiv.org/abs/2507.05270)
...
### [117] [FuzzFeed: An Automatic Approach to Weakest Precondition Generation using LLMs and Fuzzing](https://arxiv.org/abs/2507.05272)
...
### [118] [ReservoirChat: Interactive Documentation Enhanced with LLM and Knowledge Graph for ReservoirPy](https://arxiv.org/abs/2507.05279)
...
### [119] [CoreCodeBench: A Configurable Multi-Scenario Repository-Level Benchmark](https://arxiv.org/abs/2507.05281)
...
### [120] [Measuring how changes in code readability attributes affect code quality evaluation by Large Language Models](https://arxiv.org/abs/2507.05289)
...
### [121] [zkSDK: Streamlining zero-knowledge proof development through automated trace-driven ZK-backend selection](https://arxiv.org/abs/2507.05294)
*William Law*
...
### [122] [ASSURE: Metamorphic Testing for AI-powered Browser Extensions](https://arxiv.org/abs/2507.05307)
...
### [123] [OASBuilder: Generating OpenAPI Specifications from Online API Documentation with Large Language Models](https://arxiv.org/abs/2507.05316)
...
### [124] [Exploring Empathy in Software Engineering: Insights from a Grey Literature Analysis of Practitioners' Perspectives](https://arxiv.org/abs/2507.05325)
...
### [125] [Tool for Supporting Debugging and Understanding of Normative Requirements Using LLMs](https://arxiv.org/abs/2507.05504)
...
### [126] [Search-based Selection of Metamorphic Relations for Optimized Robustness Testing of Large Language Models](https://arxiv.org/abs/2507.05565)
...
### [127] [TigAug: Data Augmentation for Testing Traffic Light Detection in Autonomous Driving Systems](https://arxiv.org/abs/2507.05932)
...
### [128] [Multi-Agent Debate Strategies to Enhance Requirements Engineering with Large Language Models](https://arxiv.org/abs/2507.05981)
...
### [129] [PromiseTune: Unveiling Causally Promising and Explainable Configuration Tuning](https://arxiv.org/abs/2507.05995)
...
### [130] [Model Cards Revisited: Bridging the Gap Between Theory and Practice for Ethical AI Requirements](https://arxiv.org/abs/2507.06014)
...
### [131] [CORE: Benchmarking LLMs Code Reasoning Capabilities through Static Analysis Tasks](https://arxiv.org/abs/2507.05269)
*Danning Xie,Mingwei Zheng,Xuwei Liu,Jiannan Wang,Chengpeng Wang,Lin Tan,Xiangyu Zhang*

Main category: cs.SE

TL;DR: 本文提出了CoRe，一个用于评估大型语言模型（LLMs）程序语义推理能力的高质量静态分析基准。


<details>
  <summary>Details</summary>
Motivation: 现有评测主要关注代码生成或修复的结果，缺乏对模型程序语义理解能力的考察。

Method: 构建包含12553个任务实例的CoRe基准，覆盖数据依赖、控制依赖和信息流，采用语义多样性采样策略保证任务多样性和难度。

Result: 评测10种主流LLMs，发现虽然在依赖识别表现良好，但在深层语义理解和多步推理任务上表现不足。

Conclusion: 深入分析揭示复杂控制结构和反向依赖等挑战，提出改进LLMs代码推理能力的方向。
...
### [132] [Open Source, Hidden Costs: A Systematic Literature Review on OSS License Management](https://arxiv.org/abs/2507.05270)
*Boyuan Li,Chengwei Liu,Lingling Fan,Sen Chen,Zhenlin Zhang,Zheli Liu*

Main category: cs.SE

TL;DR: 本文系统综述了80篇关于开源软件许可的论文，聚焦许可识别、风险评估和风险缓解三个方面，揭示当前研究的不足并提出未来方向建议，以促进学界和业界的软件许可风险管理。


<details>
  <summary>Details</summary>
Motivation: 现代软件开发广泛集成第三方软件组件，带来许可方面的法律和运营风险，而现有研究和工具存在显著不足，尤其面对开源许可和生成式软件工程技术的快速发展。

Method: 通过对80篇精选开源软件许可相关文献的系统综述，将研究划分为许可识别、风险评估和风险缓解三大类，分析现有解决方案的挑战并总结未来研究机遇。

Result: 系统性地揭示当前许可风险管理中的挑战和不足，为学界和工业界提供了未来研究方向和实践建议。

Conclusion: 希望该综述能促进学界与业界的交流，推动整个软件工程社区对软件许可风险的合法治理和生态系统健康发展。
...
### [133] [FuzzFeed: An Automatic Approach to Weakest Precondition Generation using LLMs and Fuzzing](https://arxiv.org/abs/2507.05272)
*Daragh King,Vasileios Koutavas,Laura Kovacs*

Main category: cs.SE

TL;DR: 本文提出结合大型语言模型（LLM）与模糊测试，通过模糊测试反馈指导LLM生成正确的程序最弱前置条件（WP），并在Java数组程序基准上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 最弱前置条件（WP）的自动生成对于程序验证和运行时错误检测等应用非常重要，当前方法需要提升生成WP的准确性和实用性。

Method: 本文引入模糊测试引导（FG）机制，利用模糊测试对LLM生成的候选WP进行近似有效性和弱性检查，并将结果反馈给LLM以优化上下文，提高WP生成质量。

Result: 实验显示LLM能够生成有效的候选WP，通过FG机制进一步增强了其生成效果，在Java确定性数组程序的基准测试中表现优异。

Conclusion: 结合LLM与模糊测试反馈的FG方法有效提升了WP生成的准确性和实用性，验证了该方向的潜力。
...
### [134] [ReservoirChat: Interactive Documentation Enhanced with LLM and Knowledge Graph for ReservoirPy](https://arxiv.org/abs/2507.05279)
*Virgile Boraud,Yannis Bendi-Ouis,Paul Bernard,Xavier Hinaut*

Main category: cs.SE

TL;DR: 本文介绍了一个结合了RAG和知识图谱的工具，旨在提升大型语言模型在ReservoirPy代码开发和领域问题回答的表现。


<details>
  <summary>Details</summary>
Motivation: 为了减少模型生成回答中的幻觉现象，并提升其在Reservoir Computing领域的事实准确性。

Method: 通过引入检索增强生成技术（RAG）和知识图谱，将外部知识融入大型语言模型，打造专门针对ReservoirPy的交互式系统，帮助用户编写、调试Python代码并获得领域知识。

Result: 该系统在代码任务上的表现优于现有的ChatGPT-4o和NotebookLM，且明显优于其基础模型Codestral-22B。

Conclusion: 结合外部知识源的定制大语言模型能够显著提升特定领域的编码和问答能力，有效减少幻觉并提高回答的准确性。
...
### [135] [CoreCodeBench: A Configurable Multi-Scenario Repository-Level Benchmark](https://arxiv.org/abs/2507.05281)
*Lingyue Fu,Hao Guan,Bolun Zhang,Haowei Yuan,Yaoming Zhu,Jun Xu,Zongyu Wang,Lin Qiu,Xunliang Cai,Xuezhi Cao,Weiwen Liu,Weinan Zhang,Yong Yu*

Main category: cs.SE

TL;DR: 本文提出了CorePipe自动化流水线和CoreCodeBench多场景仓库级基准测试，用于更真实地评估大型语言模型在工程代码处理中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试场景单一，难以反映真实软件项目的复杂性，且问题定位和测试可靠性有限。

Method: 设计全自动流水线CorePipe生成三种原子问题（开发、修复、测试驱动开发），并组合成多种复合问题，通过调节超参数调整难度，构建多场景基准CoreCodeBench。

Result: 通过在16个大型语言模型上进行多场景测试，揭示了模型在实际工程中的多维能力差异。

Conclusion: CorePipe和CoreCodeBench为深入理解和评估大型语言模型在工程代码处理领域提供了全面且灵活的工具和基准。
...
### [136] [Measuring how changes in code readability attributes affect code quality evaluation by Large Language Models](https://arxiv.org/abs/2507.05289)
*Igor Regis da Silva Simoes,Elaine Venson*

Main category: cs.SE

TL;DR: 本文研究了使用大语言模型评估代码可读性的方法，通过对九个模型进行三种代码修改干预实验，验证其敏感性和一致性。


<details>
  <summary>Details</summary>
Motivation: 代码可读性是代码质量的重要方面，但其测量在工业和学术界存在挑战，现有工具多依赖于静态分析和主观的代码审查。

Method: 设计准实验，采用去除注释、替换标识符名称与重构去除代码异味三种干预，测试九个大语言模型对代码可读性评估的响应及其变化，结合主题分析验证模型解释能力。

Result: 所有模型对干预敏感，且与参考模型在原始及重构代码场景下高度一致；大语言模型表现出更强的语义敏感性和一定的响应波动。

Conclusion: 大语言模型在评估代码的语义质量（如标识符、注释与代码目的的连贯性）方面显示出潜力，能够实现标准化、一致性的代码可读性评价。
...
### [137] [zkSDK: Streamlining zero-knowledge proof development through automated trace-driven ZK-backend selection](https://arxiv.org/abs/2507.05294)
*William Law*

Main category: cs.SE

TL;DR: zkSDK通过抽象后端复杂性，利用定制的Python类似语言Presto和动态选择算法，自动选择最优的零知识证明后端，提升开发体验。


<details>
  <summary>Details</summary>
Motivation: 现有众多零知识证明工具导致开发者面临选择困难和碎片化体验，且通常被单一后端绑定。

Method: 提出模块化框架zkSDK，核心是Presto语言，实现程序分析与动态选择最优后端的算法。

Result: 通过对实际工作负载的深入分析和评估，验证zkSDK能有效选择最合适的后端。

Conclusion: zkSDK简化了零知识应用开发，提升开发者体验，促进多后端的灵活选择。
...
### [138] [ASSURE: Metamorphic Testing for AI-powered Browser Extensions](https://arxiv.org/abs/2507.05307)
*Xuanqi Gao,Juan Zhai,Shiqing Ma,Siyi Xie,Chao Shen*

Main category: cs.SE

TL;DR: 提出了ASSURE框架，为AI驱动的浏览器扩展提供自动化测试，以解决传统测试方法不能应对的非确定性行为和上下文敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 现有浏览器扩展测试和LLM测试方法无法有效覆盖AI驱动扩展的复杂行为和上下文依赖，导致测试效果不足。

Method: 设计了一个包含模块化测试用例生成、自动执行框架和可配置验证流水线的测试系统，系统评估行为一致性和安全漏洞，避免严格输出匹配。

Result: 在六个主流AI浏览器扩展上测试，发现531个安全漏洞和行为问题，测试效率比传统手动方法提高6.4倍，平均12.4分钟发现关键漏洞。

Conclusion: ASSURE为AI浏览器扩展测试提供了一套高效、系统化的解决方案，适合集成于开发流程中，显著提升测试的全面性与效率。
...
### [139] [OASBuilder: Generating OpenAPI Specifications from Online API Documentation with Large Language Models](https://arxiv.org/abs/2507.05316)
*Koren Lazar,Matan Vetzler,Kiran Kate,Jason Tsay,David Boaz Himanshu Gupta,Avraham Shinnar,Rohith D Vallam,David Amid Esther Goldbraich,Guy Uziel,Jim Laredo,Ateret Anaby Tavor*

Main category: cs.SE

TL;DR: 本论文提出了OASBuilder框架，用于将非结构化的API文档自动转换为结构化的机器可读API规范。


<details>
  <summary>Details</summary>
Motivation: 当前网页上的API文档多为非结构化的HTML格式，手动转换为结构化格式耗时费力，阻碍了AI代理和业务自动化工具的高效使用。

Method: 设计了一个结合大语言模型和基于规则算法的处理流程，利用文档网页的领域知识，将多样化的长API文档转换为一致的API规范。

Result: OASBuilder在数百个API测试中表现出良好的泛化能力，生成的OpenAPI规范有效且包含大部分原始文档信息。

Conclusion: OASBuilder已在企业环境中成功应用，显著节省了人工转换时间，促进了复杂企业API作为工具集成进大语言模型的可用性。
...
### [140] [Exploring Empathy in Software Engineering: Insights from a Grey Literature Analysis of Practitioners' Perspectives](https://arxiv.org/abs/2507.05325)
*Lidiany Cerqueira,João Pedro Bastos,Danilo Neves,Glauco Carneiro,Rodrigo Spínola,Sávio Freire,José Amancio Macedo Santos,Manoel Mendonça*

Main category: cs.SE

TL;DR: 本研究从实践者角度探讨软件工程中的同理心，定义其含义、识别障碍、总结实践方法及其效果，提出概念框架并通过问卷验证。


<details>
  <summary>Details</summary>
Motivation: 同理心作为关键社交技能在软件工程中重要但研究不足，需深入理解及应用。

Method: 对55篇开发者社区文章进行质性内容分析，并辅以专家问卷调查验证。

Result: 提出软件工程中同理心定义，识别有害文化和技术导向等障碍，总结促进团队同理心的实践，发现提升协作沟通及减轻压力等积极效果。

Conclusion: 构建了清晰有价值的同理心概念框架，提高了从业者意识，建议将其融入培训，未来将探索更广泛的影响。
...
### [141] [Tool for Supporting Debugging and Understanding of Normative Requirements Using LLMs](https://arxiv.org/abs/2507.05504)
*Alex Kleijwegt,Sinem Getir Yaman,Radu Calinescu*

Main category: cs.SE

TL;DR: 该论文提出了SLEEC-LLM工具，利用大型语言模型帮助非技术用户理解规范性需求（SLEEC）中规则不一致的模型检测反例，提高需求获取和一致性分析的效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 规范性需求涉及社会、法律、伦理、同理心和文化等多方面规范，通常由非技术领域的多专业利益相关者定义，需求获取过程复杂且易出错。现有基于形式方法的规则一致性验证结果对非技术用户不友好，阻碍需求迭代和验证。

Method: 提出SLEEC-LLM工具，结合大型语言模型对形式化验证工具生成的模型检测反例进行自然语言解释，从而帮助非技术利益相关者理解规则不一致问题。

Result: 通过两个真实案例验证了SLEEC-LLM的有效性，显示该工具能够提升规范性需求获取及一致性分析过程的效率和解释性。

Conclusion: SLEEC-LLM成功桥接了形式方法验证结果与非技术用户之间的沟通鸿沟，促进了多专业背景下规范性需求的高效、准确和易理解的管理。
...
### [142] [Search-based Selection of Metamorphic Relations for Optimized Robustness Testing of Large Language Models](https://arxiv.org/abs/2507.05565)
*Sangwon Hyun,Shaukat Ali,M. Ali Babar*

Main category: cs.SE

TL;DR: 本文提出了一种基于搜索算法的变异关系（MR）选择优化方法，以提升大语言模型（LLMs）鲁棒性测试的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于变异关系的LLM鲁棒性测试面临MR数量庞大且选择效率低下的问题，大多数研究仅限于单一扰动测试，测试空间受限。

Method: 本文设计并实现了四种搜索算法（Single-GA, NSGA-II, SPEA2, MOEA/D）用于优化MR组合，涵盖组合扰动，扩大测试空间，并进行比较实验。

Result: 实验表明MOEA/D算法在优化MR选择上表现最佳，并发现了具备广泛欺骗作用的关键变异关系（银弹MR），有效提升了对不同文本转文本任务的鲁棒性测试能力。

Conclusion: 本研究为LLM鲁棒性测试中的MR选择问题提供了优化方案和实证支持，推动了基于搜索的测试方法在该领域的应用与发展。
...
### [143] [TigAug: Data Augmentation for Testing Traffic Light Detection in Autonomous Driving Systems](https://arxiv.org/abs/2507.05932)
*You Lu,Dingji Wang,Kaifeng Huang,Bihuan Chen,Xin Peng*

Main category: cs.SE

TL;DR: 本文提出了TigAug，一种自动增强标注交通灯图像的方法，用于自动驾驶系统中交通灯检测模型的测试和性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有交通灯检测模型测试主要依赖手动收集和标注数据，费时且难以收集多样化环境下的数据，影响模型的可靠性和鲁棒性。

Method: TigAug基于天气环境、相机属性和交通灯属性，构建两类变异关系和三类图像变换方法，自动生成增强图像，用于检测模型错误行为并通过再训练提升模型性能。

Result: 在四种先进检测模型和两个数据集的大规模实验中，TigAug在测试效果、图像合成效率和生成图像的自然度方面表现出色。

Conclusion: TigAug有效且高效地实现了自动驾驶交通灯检测模型的自动化测试和性能提升，解决了传统方法的瓶颈问题。
...
### [144] [Multi-Agent Debate Strategies to Enhance Requirements Engineering with Large Language Models](https://arxiv.org/abs/2507.05981)
*Marc Oriol,Quim Motger,Jordi Marco,Xavier Franch*

Main category: cs.SE

TL;DR: 本文提出多代理辩论（MAD）策略用于提高大型语言模型在需求工程任务中的性能，研究表明MAD能增强分类任务的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多将模型视为黑箱，缺乏迭代完善和多视角协作，限制了模型的表现和适应性。借鉴人类辩论提升准确性和减少偏差的经验，作者提出利用多代理辩论提升LLM的表现。

Method: 系统研究不同领域的多代理辩论策略，归纳其关键特征，构建分类体系，并实现了一个基于MAD的需求工程分类框架进行初步测试。

Result: 归纳出多种MAD策略及其核心属性，初步评估显示MAD应用于需求工程分类是切实可行的。

Conclusion: 多代理辩论为提升需求工程任务中大型语言模型的准确性提供了新思路，本文为未来研究和应用改进提供了基础和指导。
...
### [145] [PromiseTune: Unveiling Causally Promising and Explainable Configuration Tuning](https://arxiv.org/abs/2507.05995)
*Pengzhou Chen,Tao Chen*

Main category: cs.SE

TL;DR: 本文提出了PromiseTune，一种基于因果推断净化规则引导的软件配置调优方法，显著提升调优效果并增强结果的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现代软件系统的高配置灵活性导致配置调优成为保障性能的关键，但现有调优方法难以有效平衡探索与利用，且缺乏对优质配置区域的了解，造成调优效果和解释性的不足。

Method: PromiseTune通过学习反映配置空间中特定区域的规则，并使用因果推断净化这些规则，获得纯净的规则近似优质配置区域，从而引导调优过程重点关注这些区域，同时结合测量数据提供空间层面的解释。

Result: 在12个系统和不同预算下，PromiseTune与11种先进调优器对比，表现出42%的显著排名优势，并提供更丰富的系统特征解释。

Conclusion: PromiseTune有效缓解了探索与利用的矛盾，提高了配置调优的性能和可解释性，为复杂系统的性能优化提供了有价值的方法和见解。
...
### [146] [Model Cards Revisited: Bridging the Gap Between Theory and Practice for Ethical AI Requirements](https://arxiv.org/abs/2507.06014)
*Tim Puhlfürß,Julia Butzke,Walid Maalej*

Main category: cs.SE

TL;DR: 本论文通过分析现有文档和模型卡，发现当前 AI 模型文档主要集中于模型能力和可靠性，缺乏对伦理方面如可解释性、公平性和用户自主权的关注。


<details>
  <summary>Details</summary>
Motivation: 存在伦理要求与 AI 模型文档实践之间的差距，需要理解这一差距并提供可操作的指导。

Method: 对26项伦理和AI指南、3个模型文档框架、3项模型卡定量研究及10个实际模型卡进行主题分析，整理出43条伦理要求并构建伦理原则分类。

Result: 发现模型开发者文档重点是模型能力和可靠性，忽视了可解释性、用户自主权和公平性等伦理方面。

Conclusion: 提出了基于伦理原则的模型卡新框架，为更全面地满足伦理AI要求提供基础，有助于改进模型文档实践。
